...
--Crie uma view chamada v_depcidade que liste o nome de cada departamento com o nome da cidade
--onde este departamento está localizado.
--Após a criacao executar    retorna 36 linhas
CREATE VIEW v_depcidade(departamento_nome,cidade_nome) AS
SELECT D.nome,C.nome from tbl_departamentos as D,tbl_cidades as C  WHERE  
C.cod_cidade = D.cod_cidade;
...
CREATE VIEW v_depcidadehouston(dep_nome) AS
select departamento_nome from v_depcidade where cidade_nome = 'Houston';
...
CREATE VIEW v_opsalario(media_salario, soma_salario) AS
SELECT avg(salario), sum(salario)  FROM tbl_empregados;
...
CREATE VIEW v_empregados_salarial(nome,salario) AS
...
CREATE VIEW v_departemp(nome,qtd_empregados) AS
SELECT D.nome, count(E.cod_empregado) FROM tbl_departamentos as D INNER JOIN tbl_empregados AS E
ON D.cod_departamento = E.cod_departamento group by D.nome ORDER BY count(E.cod_empregado) DESC;
...
SELECT D.nome FROM tbl_departamentos as D
LEFT JOIN tbl_projetos as P ON D.cod_departamento = P.cod_departamento
full JOIN tbl_empregados as E ON E.cod_departamento = D.cod_departamento
GROUP BY D.nome HAVING COUNT(P.nome) > COUNT(E.nome);
